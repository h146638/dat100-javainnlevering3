...
public class Blogg {
	...
	public int finnInnlegg(Innlegg innlegg) {
		// Starter med å sette indeksen til -1, siden det er verdien som skal returneres
		// dersom
		// innlegget ikke er med i innleggtabellen.
		int indeks = -1;

		// kjører en løkke som går gjennom alle elementene i tabellen som peker på et
		// objekt.
	...
	// Definerer en metode som sjekker om et innlegg finnes i samlingen og
	// returnerer en boolean.
	public boolean finnes(Innlegg innlegg) {
		// antar først at innlegget ikke finnes i listen og setter boolean til false
		boolean finnes = false;

		// går gjennom alle element i samlingen som peker på et objekt og dersom
		// innlegget er likt
	...
	// Definerer en metode som sjekker om det er plass til flere element i
	// samlingen.
	...
	// Definerer en metode som legger til et element i samlingen og returnerer en
	// boolean.
	...
	public String toString() {
		...
		// Går så igjennom alle element som peker på et objekt og skriver dem med
		// toString metoden
	...
	public boolean leggTilUtvid(Innlegg innlegg) {
		...
		if (!finnes(innlegg) && ledigPlass()) {
			// Dersom det er plass legges innlegget til listen, booleanen endres til true og
			// nesteledig inkrementeres.
			innleggtabell[nesteledig] = innlegg;
			lagtTil = true;
			nesteledig += 1;
		}
		// Hvis innlegget finnes i tabellen, eller dersom det ikke er ledig plass så
		// sjekker vi kun om innlegget finnes i tabellen fra før.
		else if (!finnes(innlegg)) {
			// Dersom innlegget ikke finnes i listen utvider vi tabellen.
			utvid();
			// sjekker så på nytt om det er ledig plass og om innlegget allerede er i
			// tabellen
			if (!finnes(innlegg) && ledigPlass()) {
				// Dersom det er ledig plass og innlegget ikke fins fra før, legges det til i
				// listen, boolean endres til true og neste ledig inkrementeres.
				innleggtabell[nesteledig] = innlegg;
				lagtTil = true;
				nesteledig += 1;
			}
	...
	public boolean slett(Innlegg innlegg) {
		...
		if (indeks >= 0 && finnes(innlegg)) {
			...
			for (int i = 0; i < nesteledig; i++) {
				// Dersom indeksen ikke stemmer med innlegget som skal slettes sin indeks så
				// kopieres innlegget over i den nye listen og telleren inkrementeres.
				if (i != indeks) {
					nySamling[teller] = innleggtabell[i];
					teller++;
				}
			}
			// Oppdaterer objektvariabelen til den nye listen og trekker fra en fra neste
			// ledig siden et innlegg har blitt slettet, oppdaterer boolean til true
	...
	public int[] search(String keyword) {
		...
		for (int i = 0; i < nesteledig; i++) {
			...
			// Sjekker om 5. elemetet i den splittede strengen (som inneholder teksten)
			// inneholder keyword.
			if (tabell[5].contains(keyword)) {
				// legger ii så fall til indeksen i listen over indekser og inkremeterer
				// nesteLedigeInt.
